...
for i, sig in enumerate(sigI[fitKey]):
  axs.plot(rBins, sig, label=label[i], linestyle=lineStyle[i])
  axs.legend(loc='upper right', handlelength=2.2)
axs.set_xlabel(r"$\lambda$\ \ (\AA)", labelpad=labelpad)
axs.set_ylabel(r"$\sigma_I(\lambda)$\ \ (S m$^{-1}$)", labelpad=labelpad)
#plt.text(abcPos[0], abcPos[1], '(c)', transform=axs[2].transAxes,
#         horizontalalignment='left', verticalalignment='top')

#axs.set_ylim(top=0.75)
...
plt.savefig('sig.' + format, bbox_inches="tight")
