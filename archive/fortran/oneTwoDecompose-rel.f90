program oneTwoDecompose
  ...
  logical :: is_periodic
  real(8), dimension(3), parameter :: massWater = [15.999 1.008 1.008] ![O H H]
  ...
  if (num_dataarg < num_argperdata .or. mod(num_dataarg, num_argperdata) /= 0) then
    write(*,*) "Usage: $oneTwoDecompose <outfile> <infile.g96> <numFrameToRead> <skip> <maxLag (-1=max)> &
                <numWater> <numatom1> <charge1> <mass1> [<numatom2> <charge2> <mass2>...]"
  ...
  read(tmp_str, *) numWater
  numWaterAtom = numAtom*3
  ...
  allocate(vel_water(3, numFrame, numWaterAtom), stat=stat)
  if (stat /=0) then
    write(*,*) "Allocation failed: vel_water"
    call exit(1)
  end if 
  ...
  do i = 1, numFrame
    call read_trajectory(dataFileHandle, numWaterAtom + totNumAtom, is_periodic, pos_tmp, vel_tmp, cell, time(i), stat)
    if (stat /=0) then
      write(*,*) "Reading trajectory error"
      call exit(1)
    end if 
    numFrameRead = numFrameRead + 1
    vel_water(:,i,:) = vel_tmp(:, 1:numWaterAtom)
    vel(:,i,:) = vel_tmp(:, numWaterAtom+1:end)
    do j = 1, skip-1
      call read_trajectory(dataFileHandle, numWaterAtom + totNumAtom, is_periodic, pos_tmp, vel_tmp, cell, tmp_r, stat)
      if (stat > 0) then
        write(*,*) "Reading trajectory error"
        call exit(1)
      else if (stat < 0) then
  ...
  ! chage vel to relative vel by minus v_com
  allocate(v_com(3, numFrame), stat=stat)
  if (stat /=0) then
    write(*,*) "Allocation failed: v_com"
    call exit(1)
  end if 
  v_com = 0d0
  v_com = v_com + sum(vel_water(:, :, 1:numWaterAtom:3) * massWater(1), 3)
  v_com = v_com + sum(vel_water(:, :, 2:numWaterAtom:3) * massWater(2), 3)
  v_com = v_com + sum(vel_water(:, :, 3:numWaterAtom:3) * massWater(3), 3)

  do i = 1, numAtomType
    if (i==1) then
      begin = 1
    else
      begin = sum(numAtom(1:i-1)) + 1
    end if
    v_com = v_com + sum(vel(:, :, begin:sum(numAtom(1:i))) * mass(i), 3)
  end do
