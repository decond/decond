...
!          vv = sum(vel(:, k:numFrame, i) * vel(:, 1:numFrame-k+1, j), 1)
          vv = 0d0
          vv(1:numFrame-k+1) = sum(vel(:, k:numFrame, i) * vel(:, 1:numFrame-k+1, j), 1)
          do n = 1, numFrame-k+1
            !tmp_i = rBinIndex(n)
            tmp_i = 1
...
call create_octave(vh, trim(adjustl(outFilename)) // ".data")
call write_octave_mat3(vh, "pos", pos)
call write_octave_mat3(vh, "vel", vel)
call close_octave(vh)
